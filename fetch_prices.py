...
def find_product_link(soup: BeautifulSoup, base: str) -> Optional[str]:
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"].lower()
        # Limit recognised product links to URLs that clearly reference a
        # product page; avoid generic navigation links such as "Shop" which
    ...
    try:
        ...
        if product_url:
            ...
            if list_price is not None and list_price > 0:
                return store["name"], product_url, list_price, driver
    except Exception:
        pass
